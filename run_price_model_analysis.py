...
SEED = 42
VERSION = 'seoul-price-v2-external-json'
...
def feature_inventory(f):
    source={
      'ln_capacity':('LISTING.person_capacity','Natural log of positive maximum capacity','Capacity is a maximum, not actual occupancy; mechanically part of robustness denominator.'),
      'person_capacity':('LISTING.person_capacity','Positive numeric maximum capacity; stored only, log used in models','No target information used.'),
    ...
    for col,(src,rule,caveat) in source.items():
        ...
        row={'feature':col,'source':src,'transformation':rule,'missing_n':int(f[col].isna().sum()),
             ...
             'decision':'Audit only' if col in ['district','person_capacity'] else 'Candidate; selection uses training CV', 'caveat':caveat}
        inv.append(row)
...
def initialize_tables(c,f):
    ...
    for table in TABLES:
        ...
        for info,(_,_,desc) in zip(infos,SCHEMA[table]):
...
def build_new_listing_features(c):
    ...
    for record in new:
        ...
        row={
            'listing_id':str(record['id']), 'ln_capacity':np.log(float(record['personCapacity'])),
...
def main():
    ...
    core=['ln_capacity','room_type','cluster_id']
...
def external_evaluation():
    ...
    core=['ln_capacity','room_type','cluster_id']; structure=core+['property_type','bedrooms','beds','bathrooms','amenity_count']
